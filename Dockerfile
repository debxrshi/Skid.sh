...
RUN apt update -y && \
    apt install -y nmap python3 python3-pip unzip curl screen && \
...
RUN go install -v github.com/projectdiscovery/pdtm/cmd/pdtm@latest && go install -v github.com/owasp-amass/amass/v4/...@master && go install -v github.com/OJ/gobuster/v3@latest && go install github.com/lc/gau/v2/cmd/gau@latest && go install github.com/hahwul/dalfox/v2@latest
...
RUN git clone https://github.com/m4ll0k/SecretFinder.git ~/.local/bin/gitsecretfinder && cd ~/.local/bin/gitsecretfinder && pip install -r requirements.txt --break-system-packages && cp SecretFinder ~/.local/bin/secretfinder && chmod +x ~/.local/bin/secretfinder
